...
func (r *ServiceClaimReconciler) ensureServiceClaimIsValid(
	...
	if len(pp) > 0 {
		...
		return false, r.updateServiceClaimStatusConditions(ctx, sclaim, c)
	}
...
func (r *ServiceClaimReconciler) updateServiceClaimStatusConditions(
	ctx context.Context,
	sclaim *primazaiov1alpha1.ServiceClaim,
...
func (r *ServiceClaimReconciler) getUpdatedImmutableProperties(
	...
	if sclaim.Status.OriginalServiceClassIdentity != nil &&
		!slices.Equals(sclaim.Status.OriginalServiceClassIdentity, sclaim.Spec.ServiceClassIdentity) {
		pp = append(pp, ".spec.serviceClassIdentity")
	}

	if sclaim.Status.OriginalServiceEndpointDefinitionKeys != nil &&
		!slices.Equals(sclaim.Status.OriginalServiceEndpointDefinitionKeys, sclaim.Spec.ServiceEndpointDefinitionKeys) {
