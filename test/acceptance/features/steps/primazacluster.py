...
class PrimazaCluster(Cluster):
    ...
    def read_custom_resource_status(self, group, version, plural, name):
        api_client = self.get_api_client()
        namespace = self.primaza_namespace
...
@step(u'On Primaza Cluster "{primaza_cluster_name}", the status of ServiceClaim is "{status}"')
def ensure_status_of_service_claim(context, primaza_cluster_name: str, status: str):
    primaza_cluster = context.cluster_provider.get_primaza_cluster(primaza_cluster_name)
    group = "primaza.io"
    version = "v1alpha1"
    plural = "serviceclaims"
    name = "sc-test"
    response = primaza_cluster.read_custom_resource_status(group, version, plural, name)
