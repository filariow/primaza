...
@step(u'On Primaza Cluster "{primaza_cluster}" application "{app_name}" is running in namespace "{namespace}"')
...
@step(u'On Worker Cluster "{worker_cluster}" application "{app_name}" is running in namespace "{namespace}"')
...
@step(u'On Primaza Cluster "{cluster}", application namespace "{namespace}" exists')
...
@step(u'On Worker Cluster "{worker_cluster}" application namespace "{namespace}" exists')
def namespace_is_created_worker_cluster(context, worker_cluster, namespace):
    with tempfile.NamedTemporaryFile() as tf:
        kubeconfig = context.cluster_provider.get_worker_cluster(worker_cluster).get_admin_kubeconfig()
        tf.write(kubeconfig.encode("utf-8"))
        tf.flush()
