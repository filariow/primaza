            ...
            selector:
              byLabels:
                matchLabels:
                  ...
                  c: d
        """
        And On Cluster "worker", jsonpaths are satisfied
            | jsonpath                                                          | resource type            | name    | namespace    | value          |
            | .status.conditions[] \| select(.type=="ValidResource") \| .reason | serviceclaims.primaza.io | sc-test | applications | ForbiddenField |
            | .status.conditions[] \| select(.type=="ValidResource") \| .status | serviceclaims.primaza.io | sc-test | applications | "False"        |
